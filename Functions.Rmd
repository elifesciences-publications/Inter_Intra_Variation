...
summary_2fixedeffects(data.sc_r, mm_vsris_housing, "housing")
```{r}
summary_2fixedeffects <- function(df, model, name) {
  model <- enquo(model)
  sum_name <- sym(paste0(quo_name(name), "_sum"))
  ngrps_name <-sym(paste0(quo_name(name), "_ngrps"))
  nobs_name <- sym(paste0(quo_name(name), "_nobs"))
  ANOVA_name <- sym(paste0(quo_name(name), "_ANOVA"))
  dv_name <- sym(paste0(quo_name(name), "_dv"))
  housing_name <- sym(paste0(quo_name(name), "_housing"))
  dv_housing_name <- sym(paste0(quo_name(name), "_dv_housing"))
  dv_adj_name <- sym(paste0(quo_name(name), "_adj_dv"))
  housing_adj_name <- sym(paste0(quo_name(name), "_adj_housing"))
  dv_housing_adj_name <- sym(paste0(quo_name(name), "_adj_dv_housing"))
  ...
  mutate(df,
         ...
         !! housing_name := map_dbl(!! ANOVA_name, ~.$`Pr(>Chisq)`[[2]]),
         !! dv_housing_name := map_dbl(!! ANOVA_name, ~.$`Pr(>Chisq)`[[3]]),
         !! dv_adj_name := 1,
         !! housing_adj_name := 1,
         !! dv_housing_adj_name := 1
         )
}
```
