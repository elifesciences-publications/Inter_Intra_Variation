...
threshold_vals <- calc_thresholds(sim_tib$gap_delta, cut_off = 0.95)
```

Calculate gap statistics for all features.
```{r}
...
Calculate delta gap for all features
```{r}
# df should be test_data_r$gap
diff_calc_helper <- function(df, K.max, tvs){
  gd <- tibble(clus_num = c(2:(K.max)), gap_diff = diff(df$gap))
  gd$gap_threshold <- tvs
  gd
}

test_data_r <- mutate(test_data_r,
                      gap_diff = pmap(list(gap, K.max, list(threshold_vals)), diff_calc_helper))
```

Calculate K_est for all features
```{r}
