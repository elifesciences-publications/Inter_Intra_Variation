...
clusGap_AllData <- function(df, K_max = 8) {
  df <- select(df, vm:fi) %>%
    gather("property", "value", vm:fi) %>%
    group_by(property) %>%
    nest() %>%
    mutate(gap = map2(data, K_max, clusGap_Extra_helper),
           gap_diff = pmap(list(gap, K_max, list(threshold_vals)), diff_calc),
           K_est = map_dbl(gap_diff, calc_K_est)) %>%
    select(-data)
}

# restrict to data with n ≥ 30.
all_data_r <- data.sc %>%
  group_by(id) %>%
  nest()
...
all_data_r <- filter(all_data_r, n >= 40) %>%
  mutate(clusGap = map2(data, K.max, clusGap_AllData))
```
