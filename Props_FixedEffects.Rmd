...
age_table <- FE_table(data.sc_r_f2, "age")
...
```{r}
plotFE_sd(filter(data.sc_r_f2, property == "ir")$mm_age[[1]]) +
  ggtitle("Fixed effects for input resistance")

plotFE_sd(filter(data.sc_r_f2, property == "sag")$mm_age[[1]]) +
  ggtitle("Fixed effects for the sag response")

plotFE_sd(filter(data.sc_r_f2, property == "resf")$mm_age[[1]]) +
  ggtitle("Fixed effects for membrane resonance frequency")

plotFE_sd(filter(data.sc_r_f2, property == "resmag")$mm_age[[1]]) +
...
```{r}
data.sc_r_f2_f <- filter(data.sc, age > 32) %>%
  dplyr::select(vm:fi, dvlocmm, id, housing, id, mlpos, hemi, age, housing, expr, patchdir, rectime) %>%
  gather("property", "value", vm:fi) %>%
  group_by(property) %>% 
  nest()

data.sc_r_f2_f <- data.sc_r_f2_f  %>%
  mutate(mm_vsris_age = map(data, model_vsris_age))

data.sc_r_f2_f <- summary_2fixedeffects(data.sc_r_f2_f, mm_vsris_age, "age")

data.sc_r_f2_f$mm_age_dv_adj <- p.adjust(data.sc_r_f2_f$mm_age_dv, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f$mm_age_age_adj <- p.adjust(data.sc_r_f2_f$mm_age_age, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f$mm_age_dv_age_adj <- p.adjust(data.sc_r_f2_f$mm_age_dv_age, method = "BH") %>%
  format(digits = 3)

age_f_table <- FE_table(data.sc_r_f2_f, "age")
...
```{r}
plotFE_sd(filter(data.sc_r_f2_f, property == "vm")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for Vm")

plotFE_sd(filter(data.sc_r_f2_f, property == "ir")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for input resistance")

plotFE_sd(filter(data.sc_r_f2_f, property == "sag")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for the sag response")

plotFE_sd(filter(data.sc_r_f2_f, property == "resf")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for membrane resonance frequency")
```

Look at age including only animals above P32 and below P45.
```{r}
data.sc_r_f2_f2 <- filter(data.sc, age > 32 & age < 45) %>%
  dplyr::select(vm:fi, dvlocmm, id, housing, id, mlpos, hemi, age, housing, expr, patchdir, rectime) %>%
  gather("property", "value", vm:fi) %>%
  group_by(property) %>%
  nest()
...
data.sc_r_f2_f2 <- data.sc_r_f2_f2  %>%
  mutate(mm_vsris_age = map(data, model_vsris_age)) 

data.sc_r_f2_f2 <- data.sc_r_f2_f2  %>%
  mutate(mm_age = map(data, model_vsris_age))

data.sc_r_f2_f2 <- summary_2fixedeffects(data.sc_r_f2_f2, mm_age, "age")

data.sc_r_f2_f2$mm_age_dv_adj <- p.adjust(data.sc_r_f2_f2$mm_age_dv, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_age_age_adj <- p.adjust(data.sc_r_f2_f2$mm_age_age, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_age_dv_age_adj <- p.adjust(data.sc_r_f2_f2$mm_age_dv_age, method = "BH") %>%
  format(digits = 3)

age_f2_table <- FE_table(data.sc_r_f2_f2, "age")
...
```{r}
plotFE_sd(filter(data.sc_r_f2_f2, property == "vm")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for Vm")

plotFE_sd(filter(data.sc_r_f2_f2, property == "ir")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for input resistance")

plotFE_sd(filter(data.sc_r_f2_f2, property == "sag")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for the sag response")

plotFE_sd(filter(data.sc_r_f2_f2, property == "resf")$mm_vsris_age[[1]]) +
  ggtitle("Fixed effects for membrane resonance frequency")
```
...
```{r}
df_age_table <- cbind(select(data.sc_r_f2[1:12,],
                         property,
                         mm_age_dv_adj,
                         mm_age_age_adj,
                         mm_age_dv_age_adj,
                         mm_age_ngrps,
                         mm_age_nobs),
                      select(data.sc_r_f2_f[1:12,],
                         mm_age_dv_adj,
                         mm_age_age_adj,
                         mm_age_dv_age_adj,
                         mm_age_ngrps,
                         mm_age_nobs),
                      select(data.sc_r_f2_f2[1:12,],
                         mm_age_dv_adj,
                         mm_age_age_adj,
                         mm_age_dv_age_adj,
                         mm_age_ngrps,
...
data.sc_r_f2_f2 <- summary_2fixedeffects(data.sc_r_f2_f2, mm_vsris_housing, "housing")

data.sc_r_f2_f2$mm_housing_dv_adj <- p.adjust(data.sc_r_f2_f2$mm_housing_dv, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_housing_housing_adj <- p.adjust(data.sc_r_f2_f2$mm_housing_housing, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_housing_dv_housing_adj <- p.adjust(data.sc_r_f2_f2$mm_housing_dv_housing, method = "BH") %>%
  format(digits = 3)

(housing_table <- FE_table(data.sc_r_f2_f2, "housing"))
...
```{r}
plotFE_sd(filter(data.sc_r_f2_f2, property == "sag")$mm_vsris_housing[[1]]) +
  ggtitle("Fixed effects for the sag response")

plotFE_sd(filter(data.sc_r_f2_f2, property == "spkhlf")$mm_vsris_housing[[1]]) +
...
data.sc_r_f2_f2 <- summary_2fixedeffects(data.sc_r_f2_f2, mm_vsris_ml, "ml")

data.sc_r_f2_f2$mm_ml_dv_adj <- p.adjust(data.sc_r_f2_f2$mm_ml_dv, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_ml_ml_adj <- p.adjust(data.sc_r_f2_f2$mm_ml_ml, method = "BH") %>%
  format(digits = 3)
data.sc_r_f2_f2$mm_ml_dv_ml_adj <- p.adjust(data.sc_r_f2_f2$mm_ml_dv_ml, method = "BH") %>%
  format(digits = 3)

(ml_table <- FE_table(data.sc_r_f2_f2, "ml"))
...
```{r}
plotFE_sd(filter(data.sc_r_f2_f2, property == "ir")$mm_vsris_ml[[1]]) +
  ggtitle("Fixed effects for IR")

plotFE_sd(filter(data.sc_r_f2_f2, property == "sag")$mm_vsris_ml[[1]]) +
  ggtitle("Fixed effects for membrane potential sag")

plotFE_sd(filter(data.sc_r_f2_f2, property == "tau")$mm_vsris_ml[[1]]) +
  ggtitle("Fixed effects for membrane time constant")

plotFE_sd(filter(data.sc_r_f2_f2, property == "resf")$mm_vsris_ml[[1]]) +
  ggtitle("Fixed effects for resonance frequency")

plotFE_sd(filter(data.sc_r_f2_f2, property == "ahp")$mm_vsris_ml[[1]]) +
...
```{r}
ggplot(filter(data.sc_r_f2_f2, property == "ir")$data[[1]], aes(mlpos,value, colour = dvlocmm)) +
  geom_boxplot() +
  ggtitle("Input resistance")

ggplot(filter(data.sc_r_f2_f2, property == "sag")$data[[1]], aes(mlpos,value, colour = dvlocmm)) +
  geom_boxplot() +
  ggtitle("Membrane potential sag")

ggplot(filter(data.sc_r_f2_f2, property == "resf")$data[[1]], aes(mlpos,value, colour = dvlocmm)) +
  geom_boxplot() +
...
data.sc_r_f2_f2 <- summary_2fixedeffects(data.sc_r_f2, mm_vsris_hemi, "hemi")
...
data.sc_r_f2$mm_dir_dv_adj <- p.adjust(data.sc_r_f2$mm_dir_dv, method = "BH") %>% format(digits = 3)
data.sc_r_f2$mm_dir_dir_adj <- p.adjust(data.sc_r_f2$mm_dir_dir, method = "BH") %>% format(digits = 3)
data.sc_r_f2$mm_dir_dv_dir_adj <- p.adjust(data.sc_r_f2$mm_dir_dv_dir, method = "BH") %>% format(digits = 3)
