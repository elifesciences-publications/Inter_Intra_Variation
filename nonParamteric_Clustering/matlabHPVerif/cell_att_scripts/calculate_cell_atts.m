...
switch cell_type
    case 0 % Stellate cells only 
        ...
        dv_loc_idx = 11;
    case 1 % Both
        filename = {'../../raw_data/calbindin_cell_recordings.h5'};
        dv_loc_idx = 7;
    case 2 % Both
        filename = [{'../../raw_data/stellate_cell_recordings.h5'},...
                    {'../../raw_data/calbindin_cell_recordings.h5'}];
        dv_loc_idx = [11 7];
end
...
for file = 1:length(filename)
    ...
    for m = 1:length(g1) % For each mouse...
        ...
        for I_input = 1:length(g2) % For each category of experiment...
            ...
            for c = 1:length(g2(I_input).Datasets)  % For each cell...
                ...
                if I_input == 1 % Ramp
                    ...
                    switch cell_type
                        case 0
                            cell_label{1,c} = g2(I_input).Datasets(c).Name; % Cell Label
                            cell_label{2,c} = mid(2:end); % Mouse Label
                            cell_label{3,c} = g2(I_input).Datasets(c).Attributes(3).Value; % Experimenter
                            cell_label{4,c} = g2(I_input).Datasets(c).Attributes(5).Value; % Animal Age
                            cell_label{5,c} = g2(I_input).Datasets(c).Attributes(6).Value; % Hemisphere
                            cell_label{6,c} = g2(I_input).Datasets(c).Attributes(7).Value; % Total Cells
                            cell_label{7,c} = g2(I_input).Datasets(c).Attributes(8).Value; % Housing
                            cell_label{8,c} = g2(I_input).Datasets(c).Attributes(9).Value; % Patch Direction
                            cell_label{9,c} = g2(I_input).Datasets(c).Attributes(10).Value; % Slice Position
                            cell_label{10,c} = g2(I_input).Datasets(c).Attributes(12).Value; % Recording Time
                        case 1
                            cell_label{1,c} = g2(I_input).Datasets(c).Name; % Cell Label
                            cell_label{2,c} = mid(2:end); % Mouse Label
                            cell_label{3,c} = g2(I_input).Datasets(c).Attributes(3).Value; % Experimenter
                            cell_label{4,c} = g2(I_input).Datasets(c).Attributes(5).Value; % Total Cells
                            cell_label{5,c} = g2(I_input).Datasets(c).Attributes(7).Value; % Slice Position
                    end
        ...
        clear cell_dat cell_label; % PRIOR ERROR POINT IN CELL DUPLICATION
