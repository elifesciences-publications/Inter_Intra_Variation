...
```{r}
with_var_gp <- make_real_vals(10, 1, 1)
...
ggplot(with_var_gp_data, aes(loc, param)) + geom_point(aes(colour = id))

without_var_gp_data <- make_data(without_var_gp, 20, 1, 0.2)
ggplot(without_var_gp_data, aes(loc, param)) + geom_point(aes(colour = id))
```
