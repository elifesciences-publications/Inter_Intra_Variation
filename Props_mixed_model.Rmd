...
data.sc_r <- data.sc_r %>%
  ...
  mutate(mm_vsris_age_ANOVA_dvlocmm = map(mm_vsris_age_ANOVA, ~.$`Pr(>Chisq)`[[1]])) %>%
  mutate(mm_vsris_age_ANOVA_age = map(mm_vsris_age_ANOVA, ~.$`Pr(>Chisq)`[[2]])) %>%
  mutate(mm_vsris_age_ANOVA_dvlocmm_age = map(mm_vsris_age_ANOVA, ~.$`Pr(>Chisq)`[[3]]))
```
...
kableExtra::kable(df_age_table,  col.names = c("property", rep(c("dvloc", "age", "dvloc:age", "N", "n"), 3))) %>%
