...
data.sc_TP_r <- mixedmod_extract(data.sc_TP_r, mixedmodel_vsris)
```
...
data.sc_r <- mixed_vs_linear_pchisqu(data.sc_r, "mixedmodel_vsris", "linearmodel") %>%
  mutate(pdiff_adj= 1)
data.sc_r$pdiff_adj[1:12] = p.adjust(data.sc_r$pdiff[1:12], method = "BH")
data.sc_r$pdiff_adj[13:24] = p.adjust(data.sc_r$pdiff[13:24], method = "BH")
```
...
data.sc_r_1 <- mixed_vs_linear_pchisqu(data.sc_r_1, "mixedmodel_vsris_1", "linearmodel") %>%
  mutate(pdiff_adj= 1)
data.sc_r_1$pdiff_adj[1:12] = p.adjust(data.sc_r_1$pdiff[1:12], method = "BH")
data.sc_r_1$pdiff_adj[13:24] = p.adjust(data.sc_r_1$pdiff[13:24], method = "BH")
```
...
#b <- data.sc_r$mixedmodel_vsris[[1]]
...
data.sc_r <- data.sc_r %>%
  ...
  mutate(anova_p_val_adj = 1)
data.sc_r$anova_p_val_adj[1:12] <- p.adjust(data.sc_r$anova_p_val[1:12], method = "BH")
data.sc_r$anova_p_val_adj[13:24] <- p.adjust(data.sc_r$anova_p_val[13:24], method = "BH")
```
...
data.sc_r_1 <- data.sc_r_1 %>%
  ...
  mutate(tidy_anova = map(anova, broom::tidy)) %>% 
        mutate(anova_p_val = map_dbl(tidy_anova, ~.$p.value[2]))%>%
  mutate(anova_p_val_adj = 1)
data.sc_r_1$anova_p_val_adj[1:12] <- p.adjust(data.sc_r_1$anova_p_val[1:12], method = "BH")
data.sc_r_1$anova_p_val_adj[13:24] <- p.adjust(data.sc_r_1$anova_p_val[13:24], method = "BH")
```
...
props_for_table <- c("property", "anova_p_val", "anova_p_val_adj", "marginal.r2", "conditional.r2", "pdiff", "pdiff_adj", "gradient_slopes", "modelslope_min", "modelslope_max")
...
IS_figure <- ggplot(combined_intercepts_slopes, aes(x = measure, y = value_1)) +
...
ggsave("I_S_figure.png", width = 220, height = 120, units = "mm")
```
