...
```{r}
labels_intercepts <- c(ahp = "AHP (mV)", fi = "F-I (Hz / pA)", ir = "IR (MΩ)", resf = "Res F (Hz)", resmag = "Res mag", rheo = "Rheobase (pA)", sag = "Sag", spkhlf = "Spike half-width (ms)", spkmax = "Spike maximum (mV)", spkthr = "Spike Threshold (mV)", tau = "Tm (ms)", vm = "Vrest (mV)")
...
```{r}
full_figure <- ggarrange(intercept_histograms, sagplot_c, slope_histograms, sagplot, ncol = 2, nrow = 2, widths = c(2, 1))
full_figure
```
