...
data.sc_r <- data.sc_r %>%
  ...
  mutate(mixedmodel_vsris_2r = map(data, model_vsris_2r))
```

Use step to remove random effects.
****** Warning: this takes several hours to run for vsris_all *******
```{r}
data.sc_r <- data.sc_r %>%
  mutate(vsris_step_all = map(mixedmodel_vsris_all, lmerTest::step)) %>%
  mutate(vsris_step_2r = map(mixedmodel_vsris_2r, lmerTest::step))
```
