...
## Evalaute impact of other experimental parameters
...
Build models with all possible random effects.
```{r}
data.sc_r <- data.sc_r %>%
  mutate(mixedmodel_vsris_all = map(data, model_vsris_all)) 
```

Use step to remove random effects. Then extract models identified with lmerTest::step
...
Output from step indicates some models failed to converge. The larger model (mixedmodel_vsris_all) does converge. Failure appears to be associated with random effects with very small variance. Normalisation doesn't fix the problem. To check whether the selected models can be improved, run step again to further evaluate random effects and extract models identified. 
```{r}
data.sc_r <- data.sc_r %>%
...
Extract p-values for (1|id) and (0 + dvlocmm | id) estimated using the step model.
```{r}
data.sc_r <- data.sc_r %>%
  mutate(vsris_step_random = map(vsris_step_2, ~.$random)) %>%
  mutate(vsris_step_row = map(vsris_step_random, rownames_to_column))
```

Make tables with summaries of step models.
...
out.pca.x_g <- out.pca.x_g %>%
  ...
  mutate(vsris_null_glance = map(mixedmodel_vsris_null, broom::glance)) %>%
  mutate(vsri_glance = map(mixedmodel_vsri, broom::glance)) %>%
  mutate(vscris_tidy = map(mixedmodel_vsris, broom::tidy)) %>%
  mutate(vscris_aug = map(mixedmodel_vsris, broom::augment)) %>%
  mutate(vscris_summary = map(mixedmodel_vsris, summary))
```
